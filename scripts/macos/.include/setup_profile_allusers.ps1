...
.EXAMPLE
sudo scripts/macos/.include/setup_profile_allusers.ps1 -UserName $(id -un)
#>
param (
...
begin {
    ...
    if (Test-Path .config/pwsh_cfg -PathType Container) {
        if (-not (Test-Path $CFG_PATH)) {
            sudo -u $user mkdir -p $CFG_PATH
        }
        Copy-Item .config/pwsh_cfg/* $CFG_PATH -Force
    }
}
...
process {
    ...
    if (Test-Path $CFG_PATH/_aliases_macos.ps1) {
        ...
        $content = [IO.File]::ReadAllLines("$CFG_PATH/_aliases_macos.ps1").Replace('exa -g ', "exa -g $exa_param")
        [IO.File]::WriteAllLines("$CFG_PATH/_aliases_macos.ps1", $content)
    }
    ...
    if (Test-Path $CFG_PATH -PathType Container) {
        ...
        install -m 0644 $CFG_PATH/_aliases_macos.ps1 $SCRIPTS_PATH
        # clean config folder
    ...
    if (Get-InstalledModule -Name Microsoft.PowerShell.PSResourceGet) {
        ...
        if (Test-Path scripts/macos/.include/update_psresources.ps1 -PathType Leaf) {
            scripts/macos/.include/update_psresources.ps1
        }
    }
}
